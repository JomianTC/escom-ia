      ...
      <ul className={`flex flex-col-reverse sm:flex-row gap-6 justify-center items-center relative ${showNav ? 'active' : ''}`} >
        ...
        </li>
        {
          NAV_ROUTES.map((route) => (
            <li key={route.path} className='rounded-full border-2 border-primary_200 sm:rounded-none sm:border-0 absolute bottom-0 sm:relative p-2' onClick={handleToggleNavBar} >
              <NavLink to={route.navPath} className={({ isActive }: { isActive: boolean }) => `nav__link ${isActive ? 'active text-primary_300 font-bold' : ''} `} end>
                ...
                <img className={`w-10 h-10 bg-cover sm:hidden ${route.name !== 'Chatbot' ? 'invert' : ''}`} src={route.imageUri} alt="escom_plus_logo" />
              </NavLink>
            </li>
          ))
        }
        {showControls && (
          <>
            {rol === LEVEL_ACCESS.ADMIN
              ? (<NavLink to={'dashboardadmin'} onClick={handleToggleNavBar} className={({ isActive }: { isActive: boolean }) => `nav__link rounded-full border-2 border-primary_200 sm:rounded-none sm:border-0 absolute bottom-0 sm:relative p-2  ${isActive ? 'active text-primary_300 font-bold ' : ''} `} end>
