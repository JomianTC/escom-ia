        ...
        <div className='fixed w-full h-screen top-0 left-0 flex justify-center items-center bg-zinc-400/80  z-[9000] p-8'>
          <div className='bg-bg_300 p-4 rounded-lg text-text_100 flex flex-col gap-8'>
            ...
            <div className='flex gap-4 justify-end mt-8'>
                <button onClick={async () => { await handleRemoveTeacher() }} className='bg-accent_200 text-bg_300 p-2 rounded-lg'>Eliminar</button>
            ...
            {infoModal.type === 'tag' && (
                <div >
                <p className='text-base md:text-lg '>¿Estas seguro de eliminar el tag {infoModal?.nombre }?</p>
                <div className='flex gap-4 justify-end mt-8'>
                    <button onClick={async () => { await handleRemoveTag() }} className='bg-accent_200 text-bg_300 p-2 rounded-lg'>Eliminar</button>
                    <button onClick={async () => { dispatch(closeDeleteModal()) }} className='bg-accent_100 text-bg_300 p-2 rounded-lg px-3'>Cancelar</button>
                </div>
                </div>
            )}
