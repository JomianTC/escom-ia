  ...
  return (
    <article key={uuid()} className='white-border flex relative h-48  text-white comment-box overflow-hidden z-30 gap-2 sm:gap-4 '>
      ...
      <div className='flex flex-col gap-2 basis-20 sm:basis-32 min-w-20 md:min-w-32 '>
        <img className={`w-14 min-h-14  md:w-16 md:h-16 rounded-full overflow-hidden ${active && 'opacity-0'}`} src={getProfilePicture(comment.usuario.foto_perfil)} alt={comment.usuario.nombres} />
        <div className={`gap-1 custom-scrollbar transition-all ${active ? ' flex flex-row flex-wrap absolute top-0 bg-accent_100 w-full h-full left-0 white-border items-start justify-start content-start gap-2 ' : 'items-start flex flex-col sm:flex-row sm:flex-wrap '}`} >
        {showTags && (
            <span className={` cursor-pointer border-2 border-primary_100 rounded-lg px-[8px] text-xs mt-1 ${active && 'text-black'}`} onClick={() => { setActive(!active) }}>
              {active ? 'Ocultar' : 'Ver más'}
            </span>
        )}
          {(!active && showTags) && comment.tags.slice(0, 4).map((tag) => (
      ...
      {/* USER INFO */}
      <div className={`flex flex-col shrink-[4] overflow-y-scroll hide-scrollbar ${active && 'opacity-0'}`}>
