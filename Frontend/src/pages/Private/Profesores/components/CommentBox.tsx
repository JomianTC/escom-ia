...
export default function CommentBox ({ comment }: CommentBoxProps) {
  ...
  return (
    <article key={uuid()} className='white-border flex relative h-48  text-white comment-box'>
    ...
    {/* USER INFO */}
      <div className='flex flex-col'>
        <div className='flex items-center gap-6 '>
          <p className='text-3xl font-bold text-zinc-600'>{comment.usuario.nombres + ' ' + comment.usuario.apellidos}</p>
          <span className='text-sm text-zinc-600 italic'>{ comment.comentario.fecha}</span>
        </div>
        <div className='flex grow flex-col justify-between pb-6 text-zinc-600'>
