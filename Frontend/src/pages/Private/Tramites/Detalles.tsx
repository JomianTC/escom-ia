...
export function Detalles () {
      ...
      <div className='container form-glass bg-bg_200 relative mx-auto h-full z-[100] p-4'>
        <div className='backdrop-blur-3xl h-full rounded-lg overflow-hidden overflow-y-scroll hide-scrollbar'>
          <div className='flex w-full justify-between '>
            <h1 className='mx-auto font-bold text-center text-4xl sm:text-5xl md:text-6xl  text-accent_100 drop-shadow-lg'>Cargando...</h1>
            <ReturnButton styles=' '/>
          </div>
        </div>
      </div>
  ...
  const daysToExpire = diffDays(new Date(data?.fechaTermino ?? ''), new Date(data?.fechaInicio ?? '')) ?? 0
  const linksFromDesc = getAnchorTags(createMarkup(data?.descripcion ?? '').__html)

  return (
    <div className='container form-glass bg-bg_200 relative mx-auto h-full z-[100] p-4'>
      <div className='backdrop-blur-3xl h-full rounded-lg overflow-hidden overflow-y-scroll hide-scrollbar'>
        <div className='flex w-full justify-between '>
        <h1 className='mx-auto font-bold text-center text-4xl sm:text-5xl md:text-6xl  text-accent_100 drop-shadow-lg'>{data?.nombre}</h1>
        <ReturnButton styles=' '/>
        </div>
        <div className='flex h-full max-h-full overflow-hidden flex-col sm:flex-row  hide-scrollbar'>
          {/* Columna detalles  */}
          <div className={`flex flex-row justify-center md:justify-start gap-1 sm:gap-6  md:border-r-4 border-r-accent_100  sm:grow-0 grow h-fit sm:h-full  flex-wrap p-2 md:p-2 items-start content-start sm:max-w-[320px] w-full  ${data !== (undefined) && data?.requerimientos.length <= 0 && 'grow-0'}`}>
            ...
            {/* Label container */}
            <div className={`w-full flex flex-wrap gap-2 requeriments-container  overflow-hidden overflow-y-scroll hide-scrollbar px-6 py-3 sm:px-2 sm:h-full  ${data !== undefined && data?.requerimientos.length > 0 ? 'min-h-24 max-h-36 sm:max-h-full items-start content-start  ' : 'h-fit'}`}>
              <h4 className='grow w-full text-accent_100 font-bold text-3xl '>Requisitos</h4>
              {data !== undefined && data?.requerimientos?.length > 0 && data?.requerimientos.map((requerimiento: string) => (
          ...
          <div className=' grow overflow-y-scroll hide-scrollbar mt-8 px-4 py-2 w-full'>
            ...
            <div className='text-lg p-3 w-full' dangerouslySetInnerHTML={createMarkup(data?.descripcion ?? '')}></div>
